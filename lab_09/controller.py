...
class Controller:
    ...
    def add_cutter_vertex(self, v: Point) -> None:
        if self._cutter.is_closed():
            self.reset_cutter()
        try:
            edge = self.cutter.add_vertex(
                v, QGuiApp.keyboardModifiers() & Qt.ShiftModifier)
    ...
    def close_cutter(self) -> None:
        if self._cutter.is_closed():
            QMessageBox.information(
                self._canvas, "Внимание", "Отсекатель уже замкнут")
            return
        try:
    ...
    def close_poly(self) -> None:
        if self._poly.is_closed():
            QMessageBox.information(
                self._canvas, "Внимание", "Многоугольник уже замкнут")
            return
    ...
    def cut(self) -> None:
        if not self._cutter.is_closed():
            QMessageBox.critical(self._canvas, "Ошибка",
                                 "Для отсечения необходимо задать отсекатель")
            return

        if not self._poly.is_closed():
            QMessageBox.critical(self._canvas, "Ошибка",
                                 "Необходимо задать многоугольник для отсечения")
            return
    ...
    def draw_result(self, result_poly) -> None:
        self._canvas.draw_segments(
            result_poly.edges, self._result_color, is_result=True)
