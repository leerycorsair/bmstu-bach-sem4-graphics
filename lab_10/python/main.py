...
class Window(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def horizon(self, start_x, start_y, end_x, end_y):
        if end_x == start_x:
            if end_x >= self.scene.width():
                return
            self.max_horizon[end_x] = max(self.max_horizon[end_x], end_y)
            self.min_horizon[end_x] = min(self.min_horizon[end_x], end_y)
        else:
    ...
    def draw(self, x_from, x_to, x_step, z_from, z_to, z_step, f, color):
        ...
        self.scale_coeff = self.width / (x_to - x_from) * 0.6
        while z >= z_from:
            x_last = x_from
            y_last = f(x_from, z)
            z_buf = z
            ...
            if x_left != -1:
                if self.is_visible(x_left, y_left) and self.is_visible(x_last, y_last):
                    self.scene.addLine(
                        x_last, y_last, x_left, y_left, QPen(color))
            ...
            while x <= x_to:
                ...
                if prev_visibility == curr_visibility:
                    if curr_visibility:
                        self.scene.addLine(
                            x_last, y_last, x_curr, y_curr, QPen(color))
                        self.horizon(x_last, y_last, x_curr, y_curr)
                else:
                    if not curr_visibility:
                        if prev_visibility == 1:
                            x_inter, y_inter = self.intersection(
                                x_last, y_last, x_curr, y_curr, self.max_horizon)
                        else:
                            x_inter, y_inter = self.intersection(
                                x_last, y_last, x_curr, y_curr, self.min_horizon)

                        self.scene.addLine(
                            x_last, y_last, x_inter, y_inter, QPen(color))
                        ...
                        if not prev_visibility:
                            x_inter, y_inter = self.intersection(
                                x_last, y_last, x_curr, y_curr, self.max_horizon)
                            self.scene.addLine(
                                x_inter, y_inter, x_curr, y_curr, QPen(color))
            ...
            if x_right != -1 and prev_visibility:
                self.scene.addLine(x_right, y_right, x_curr,
                                   y_curr, QPen(color))
    ...
    def draw_surface(self):
        try:
            ...
            self.show_message(
                "Ошибка", "Выберите уравнение поверхности", "Ubuntu 15")
        except TypeError():
            self.show_message(
                "Ошибка", "Выберите цвет построения", "Ubuntu 15")
        except IOError:
    ...
    def function(self):
        if self.func_box.currentIndex() == 0:
            return f1
